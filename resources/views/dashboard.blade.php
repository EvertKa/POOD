<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-blue-900 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="bg-white dark:bg-blue-700 overflow-hidden shadow-sm sm:rounded-lg mt-3 p-3">
                        {{ __("Shopping cart") }}
                        @foreach($carts as $cart)
                        @if($cart->user == $user->id)
                        <div class="p-3 bg-gray-900 dark:bg-blue-600 w-auto min-h-12 h-auto rounded-lg m-2">
                            <div class="flex flex-row justify-between">
                                ...
                                <form action="{{ route('cart.destroy', $cart->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="button px-2 h-8 text-black dark:text-white bg-red-500 rounded-md" onclick="return confirm('Are you sure you want to delete this cart item?')">Delete Cart Item</button>
                                </form>
                            </div>
                        </div>
                        @endif
            ...
            <div class="bg-white dark:bg-blue-900 overflow-hidden shadow-sm sm:rounded-lg mt-3">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @foreach ($items as $item)
                    <div class="p-3 bg-gray-800 dark:bg-blue-700 w-auto min-h-12 h-auto rounded-lg m-3">
                        <div class="flex flex-row justify-between">
                            <div>
